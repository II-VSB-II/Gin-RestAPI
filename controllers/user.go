...
import (
	models "github.com/II-VSB-II/Gin-RestAPI/models"
...
func RegisterUser(c *gin.Context) {
	...
	if binderr := c.ShouldBindJSON(&user); binderr != nil {
		log.Error().Err(binderr).Str("Request ID", RequestID).
	...
	if conErr != nil {
		log.Err(conErr).Str("Request ID", RequestID).Msg(
			"Error Occurred While Getting Database Connection From The Connection Pool.")
	...
	if result.Error != nil && result.RowsAffected != 1 {
		log.Err(result.Error).Str("Request ID", RequestID).Msg("Error Occurred While Creating New User.")
